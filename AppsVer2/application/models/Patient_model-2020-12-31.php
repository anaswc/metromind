...

	{
		...
		if($this->communicationMode <> 3){	
			...
			if($this->appointmentId == 0){
				...
				$this->errorMessage =  'You have no appointment at this time. Please schedule your appointment!';

				return 0;	

			}else if($this->check_doctor_availabilty($this->doctorId)){

				$this->errorMessage = 'Doctor is not available at the moment. Please schedule your appointment!';
			...
			$communicationStartTime = strtotime(date('Y-m-d H:i:s', strtotime("$appointmentDate $appointmentStartTime")));	
			
			$serverTime 			= strtotime(date("Y-m-d H:i:s", now('Asia/Kolkata')));
			
			$diff 					= abs($communicationStartTime - $serverTime)/60;
			
			if($diff > 2){

				$this->errorMessage = 'Not a valid time to make call !!!';
...
// =======================================
	function end_communication_log_patient($patientRecordId)
	{
		$this->db->select('doctorId,communicationDuration,patientCreditId,appointmentId,communicationStartTime');
		$this->db->from("axpatientrecords");
...
// ------------
			...
			if($querys->num_rows() > 0){	
				$row_arrays = $querys->row_array();
				$doctorSessionDuration=$row_arrays['doctorSessionDuration']/2;
				$orgnlduratin=$row_arrays['doctorSessionDuration'];
			}
// print_r($pr_communicationDuration);exit;
		...
		if($communicationDuration[1]<=$orgnlduratin)
		{

			// echo $latesttime;
			$lttime= explode(':',$latesttime);
			if($lttime[1]>$orgnlduratin)
			{
		// print_r($orgnlduratin);exit;



		// $time1 = new DateTime($communicationStartTime);
		$checkTime = strtotime($communicationStartTime);
		$checkout_time = strtotime($this->communicationEndTime);
		// echo 'Check Time : '.date('H:i:s', $checkTime);exit;
		

		$time2 = $checkTime-$checkout_time  ;
		// $interval = $checkTime->diff($checkout_time);
		print_r($time2);exit;
		echo $interval->format('%s second(s)');
				$data = array(
					'communicationEndTime' 		=> $this->communicationEndTime,
					'communicationDuration' 	=> gmdate("H:i:s", $orgnlduratin*60)
		
				);
			}
			else{
		// print_r($latesttime);exit;
	...
	public function end_communication_log_patient_old($patientRecordId)
...
// Dated 05 May 2020 Dileep to make appointment request and confirmed request into a single api - get_patient_requested_fixed_appointments
	...
	{	
		...
		$this->db->order_by("status ASC, YEAR(appointmentDate) DESC, MONTH(appointmentDate) DESC, DAY(appointmentDate) DESC, appointmentStartTime ASC ");
...
//	End of get_patient_requested_fixed_appointments
	...
	{	
		...
		$this->db->select(' 
							...
							axdoctors.qualification AS specialityName,
...

	{
		...
		$this->db->select('patientId,totalAmount,doctorId');
...
// End 
	...
	{	
        ...
        if($this->communicationStartTime <> ''){
           ...
           $this->db->where('axappointments.startDateTime <= "'.$this->startDateTime.'" AND axappointments.endDateTime > "'.$this->startDateTime.'"');
	...
	{	
	 ...
	 // To get the appointment alidity of the patient for the doctor , on the specified date and time interval

	public function check_appointment_validity($patientId,$doctorId,$appointmentId)
	{	
