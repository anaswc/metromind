	...
	public function update_appointment_status_doctor($appointmentId)
	{
		...
		if($query->num_rows() > 0){	
			...
			if($this->status == 1)
				...
				$notificationTitle = 'Your online appointment with '.$doctorName.'('.$doctorUniqueId.') is booked !';

			else if($this->status == 2){

				$this->db->delete("axnotifications", "appointmentId IN ( ".$appointmentId.")");

				$notificationTitle = 'Your online appointment with '.$doctorName.'('.$doctorUniqueId.') is rejected !';
...
// End of get_doctor_requested_fixed_appointments
	...
	public function get_doctor_fcm_token($doctorId){
		...
		$this->db->select('fcmToken,chatRoomNumber,voipToken');				
