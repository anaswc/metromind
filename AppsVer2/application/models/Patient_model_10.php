...
class Patient_model_10 extends CI_Model {
...

	{
		...
		$this->db->select('appointmentId,isNewAppointment,subscriptionId,patientCreditId');
...
//	End of get_patient_requested_fixed_appointments
...

	{
		...
		$api = new Api(RAZORAPIKEY, RAZORSECRETKEY);
	...
	function sendFCM($patientId, $doctorId) {
		...
		if($loginStatus == 1){	// send push notification only if , patient or doctor is online based on the login type
			...
			}else if($this->deviceOS == 'iOS'){	// send push notification only if , patient or doctor is online based on the login type
	...
	public function get_patient_fcm_token($patientId){
		...
		$this->db->select('fcmToken,firstName,lastName,uniqueId,loginStatus');				
	...
	public function get_doctor_fcm_token($doctorId){
		...
		$this->db->select('fcmToken,chatRoomNumber,uniqueId,loginStatus,voipToken,deviceOS');				
		...
		if($query->num_rows() > 0){	
				...
				$this->deviceOS			= $row_array['deviceOS'];
	...
	public function sent_fcm_token_test(){
		...
		$fields = array (
				...
				'to' 	=> $this->input->post_get('fcmtoken'),
				...
				'notification' 	=> array (
						...
						'title' 			=> 'Your online appointment with T-Dr. Thalhath. P(METROMINDD1) is booked!',

						'doctorId' 			=> $this->input->post_get('uniqueId'),

						'appointmentId' 	=> '263',

						'appointmentDate' 	=> '08-Jul-2020',

						'appointmentStartTime' 	=> "11:15AM",

						'appointmentEndTime' 	=> "11:45 AM",

						'doctorName' 		=> "T-Dr. Thalhath. P",
	...
	public function sent_fcm_token_test_ios(){
		
		$data =  array();
		
		$passphrase = '12345';

		$deviceToken =  $this->input->post_get('fcmtoken');
	...
	public function sent_fcm_token_test_ios_2(){
		...
		//stream_context_set_option($ctx, 'ssl', 'local_cert', 'PushDevelopmentCTNX.pem');
		stream_context_set_option($ctx, 'ssl', 'local_cert', 'VOIP.pem');//live
