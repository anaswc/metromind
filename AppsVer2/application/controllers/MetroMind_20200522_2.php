...

{
...

	{
		...
		if ($this->input->post_get('userName') <> '' && $this->input->post_get('password') <> '') {
			...
			$response = ['status' => 201, 'result' => $result, 'message' => 'This email does not seem right with us. Please enter another email'];
		...
		if ($this->input->post_get('loginType') == 2) {
			...
			if (count($result) > 0) {
				...
				foreach ($result as $row) {
					...
					/*if(strtotime($result[$i]['appointmentDate']) == strtotime($this->Patient_model->currentDay)){

						

						if(strtotime($result[$i]['appointmentEndTime'])  <=  strtotime($this->Patient_model->currentDayTime)){
...

	{
		...
		if ($this->input->post_get('loginType') == 2) {
			...
			$result 	= $this->Patient_model->get_patient_requested_fixed_appointments($this->Patient_model->patientId);
			...
			if (is_array($result) && count($result) > 0) {
					...
					foreach ($result as $row) {
						...
						/*if(strtotime($result[$i]['requestDate']) == strtotime($this->Patient_model->currentDay)){

							

							$this->Doctor_model->availableDay = date('l',strtotime($result[$i]['requestDate']));
							...
							$today_available_sessions = $this->Doctor_model->get_doctor_sessions($result[$i]['doctorId']);

							

							if (count($today_available_sessions) > 0) {

								foreach ($today_available_sessions as $row1) {

									if($row1['availableSession'] == $result[$i]['requestSession']){

										

										if(strtotime($row1['availableEndTime'])  <=  strtotime($this->Patient_model->currentDayTime)){
...

	{
		...
		if ($this->Patient_model->patientId == '' || $this->Patient_model->doctorId == '' || $this->Patient_model->packageId == '') {
			...
			if (!$result) {
				...
				$response = ['status' => $status, 'token' => $token, 'result' => $result, 'doctorId' => $this->Patient_model->doctorId, 'paymentId' => $this->Patient_model->paymentId, 'paymentAmount' => $this->Patient_model->paymentAmount, 'order_id' => $this->Patient_model->order_id,  'message' => HTTP_STATUS_CODES[481]];
...

	{
		...
		if ($this->Patient_model->patientId == '' || $this->Patient_model->doctorId == '') {
			...
			if (!$result) {
				...
				$response = ['status' => $status, 'token' => $token, 'result' => $result, 'doctorId' => $this->Patient_model->doctorId, 'paymentId' => $this->Patient_model->paymentId, 'paymentAmount' => $this->Patient_model->paymentAmount, 'order_id' => $this->Patient_model->order_id, 'message' => HTTP_STATUS_CODES[200]];
		...
		if ($this->Patient_model->patientId == '' || $this->Patient_model->doctorId == '') {
			...
			if (!$result) {
				...
				if ($this->Patient_model->errorMessage <> '') {
					...
					$errorMessage	= HTTP_STATUS_CODES[200];
...

	{
		...
		if ($this->Patient_model->prescriptionId == '') {
			...
			if (!$result) {
				...
				$response = ['status' => $status, 'token' => $token, 'result' => $result, 'paymentId' => $this->Patient_model->paymentId, 'paymentAmount' => $this->Patient_model->paymentAmount, 'order_id' => $this->Patient_model->order_id, 'message' => HTTP_STATUS_CODES[200]];
...

	{
		...
		if ($this->Doctor_model->doctorId <> '') {
			...
			if (!$result) {
				...
				$response = ['status' => $status, 'token' => $token, 'result' => array(), 'doctorUniqueId'  => $result['doctorUniqueId'], 'doctorName'  => $result['doctorName'], 'doctorEmail'  => $result['doctorEmail'], 'doctorMobile'  => $result['doctorMobile'], 'qualification'  => $result['qualification'], 'doctorImageUrl'  => $result['doctorImageUrl'], 'experience'  => $result['experience'], 'knownLanguages'  => $result['knownLanguages'], 'specialization'  => $result['specialization'], 'youtubeLink'  => $result['youtubeLink'], 'doctorAddress'  => $result['doctorAddress'], 'communicationMode'  => $result['communicationMode'], 'medicalRegistrationNumber'  => $result['medicalRegistrationNumber'], 'overall_rating'  => $result['overall_rating'], 'num_appointments'  => $result['num_appointments'], 'num_patients'  => $result['num_patients'], 'total_payment'  => $result['total_payment'], 'fixed_appointments' => $fixed_appointments, 'message' => HTTP_STATUS_CODES[200]];
	...
	function update_appointment_time_post()

    {

            

            $result = $this->Patient_model->update_appointment_time(); 
