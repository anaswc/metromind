	...
	function add_communication_log_post()
	{
		...
		if ($this->Patient_model->patientId == '' || $this->Patient_model->doctorId == '' || $this->Patient_model->communicationMode == '' || $this->Patient_model->communicationStartTime == '') {
			...
			if (!$result) {
				...
				$response = ['status' => $status, 'token' => $token, 'result' => $result, 'patientRecordId' => $this->Patient_model->patientRecordId, 'doctorDeviceOS' => $this->Patient_model->doctorDeviceOS, 'patientDeviceOS' => $this->Patient_model->patientDeviceOS,  'message' => HTTP_STATUS_CODES[200]];
