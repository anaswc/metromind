...
class MetroMind_1 extends CI_Controller

{
...

	{
		...
		$this->load->model('Patient_model_10');
...

	{
		...
		if ($this->Doctor_model->doctorId <> '') {
			...
			if (!$result) {
				...
				$response = ['status' => $status, 'token' => $token, 'result' => array(), 'doctorUniqueId'  => $result['doctorUniqueId'], 'doctorName'  => $result['doctorName'], 'doctorEmail'  => $result['doctorEmail'], 'doctorMobile'  => $result['doctorMobile'], 'qualification'  => $result['qualification'], 'qualification1'  => $result['qualification1'], 'doctorImageUrl'  => $result['doctorImageUrl'], 'experience'  => $result['experience'], 'knownLanguages'  => $result['knownLanguages'], 'specialization'  => $result['specialization'], 'specialityId'  => $result['specialityId'], 'youtubeLink'  => $result['youtubeLink'], 'doctorAddress'  => $result['doctorAddress'], 'communicationMode'  => $result['communicationMode'], 'medicalRegistrationNumber'  => $result['medicalRegistrationNumber'], 'overall_rating'  => $result['overall_rating'], 'num_appointments'  => $result['num_appointments'], 'num_patients'  => $result['num_patients'], 'total_payment'  => $result['total_payment'], 'fixed_appointments' => $fixed_appointments, 'message' => $message];
...

	{
		...
		if ($this->input->post_get('recieverUniqueId') == '' || $this->input->post_get('recieverLoginType') == '' || $this->input->post_get('type') == '') {
			...
			if (!$result) {
				...
				$response = ['status' => $status, 'token' => $token, 'result' => $result, 'appointmentId' => $this->Patient_model->appointmentId, 'to' => $to, 'type' => $this->input->post_get('type'),  'message' => ''];
	...
	function sent_test_fcm_token_post()
	{

		$this->Patient_model->setPostGetVars();

		$result = $this->Patient_model_10->sent_fcm_token_test();
		
		$status = 200;

		$response = ['status' => $status, 'token' => '', 'result' => $result,  'message' => HTTP_STATUS_CODES[200]];

		$this->response($response, 200);
	}
	
	function sent_test_fcm_token_ios_post()
	{

		$this->Patient_model->setPostGetVars();

		$result = $this->Patient_model_10->sent_fcm_token_test_ios();
	...
	function sent_test_fcm_token_ios_2_post()
	{

		$this->Patient_model->setPostGetVars();

		$result = $this->Patient_model_10->sent_fcm_token_test_ios_2();
		
		$status = 200;

		$response = ['status' => $status, 'token' => '', 'result' => $result, 'length' => strlen($this->input->post_get('fcmtoken')),  'message' => HTTP_STATUS_CODES[200]];
