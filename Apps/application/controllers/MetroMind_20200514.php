...
class MetroMind extends CI_Controller
{
	...
	function signup_post()
	{
		...
		if (!$valid) {
			...
			$response = ['status' => $errorMessage,  'result' => $result, 'uniqueId' => $this->Patient_model->uniqueId, 'isCompletedInfo' => $isCompletedInfo, 'message' => HTTP_STATUS_CODES[$errorMessage]];
		...
		if (!$result) {
			...
			if (count($result) > 0) {
				...
				foreach ($result as $row) {
					...
					$result[$i]['overall_rating'] 	= round($overall_rating,1);
	...
	function doctor_details_get()
	{
		...
		if ($this->Doctor_model->doctorId <> '') {
			...
			if (!$result) {
				...
				$result['overall_rating'] 	= round($overall_rating,1);
