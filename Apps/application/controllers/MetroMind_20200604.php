...

	{
		...
		if ($this->Doctor_model->doctorId <> '') {
			...
			if (!$result) {
				...
				$response = ['status' => $status, 'token' => $token, 'result' => array(), 'doctorUniqueId'  => $result['doctorUniqueId'], 'doctorName'  => $result['doctorName'], 'doctorEmail'  => $result['doctorEmail'], 'doctorMobile'  => $result['doctorMobile'], 'qualification'  => $result['qualification'], 'doctorImageUrl'  => $result['doctorImageUrl'], 'experience'  => $result['experience'], 'knownLanguages'  => $result['knownLanguages'], 'specialization'  => $result['specialization'], 'specialityId'  => $result['specialityId'], 'youtubeLink'  => $result['youtubeLink'], 'doctorAddress'  => $result['doctorAddress'], 'communicationMode'  => $result['communicationMode'], 'medicalRegistrationNumber'  => $result['medicalRegistrationNumber'], 'overall_rating'  => $result['overall_rating'], 'num_appointments'  => $result['num_appointments'], 'num_patients'  => $result['num_patients'], 'total_payment'  => $result['total_payment'], 'fixed_appointments' => $fixed_appointments, 'message' => HTTP_STATUS_CODES[200]];
...

	{
		...
		if ($this->input->post_get('recieverUniqueId') == '' || $this->input->post_get('recieverLoginType') == '' || $this->input->post_get('type') == '') {
			...
			if (!$result) {
				...
				$response = ['status' => $status, 'token' => $token, 'result' => $result, 'to' => $to, 'type' => $this->input->post_get('type'),  'message' => ''];
