...
//Response : if the mandatory values is not filled then status = 460, if the password enterd is not in the valid format  status = 479, if the email is already used status = 461 and if the email is already used and the account is deleted status = 480 
	...
	function signup_post(){	
		...
		if($this->Patient_model->patientMobile == '' || $this->Patient_model->otp == ''){
		...
		if(!$valid){
			...
			$result = $this->Patient_model->update_otp_patient($this->Patient_model->uniqueId);
	...
	function login_post(){	
		...
		if($this->input->post_get('userName') <> '' && $this->input->post_get('password') <> ''){
			...
			if(!$result){
				...
				$response = ['status' => $status, 'token' => $token,'uniqueId' => $uniqueId,'clientName' => $clientName,'clientEmail' => $clientEmail,'clientMobile' => $clientMobile,'clientAddress' => $clientAddress,'loginType' => $loginType,'imageUrl' => $imageUrl,'youtubeLink' => $youtubeLink,'message' => 'You are in! '];
...
// Response		: if doctor unique id is empty return status = 460 (INCOMPLETE INFORMATION), other wise update the doctor basic details provided and  return status = 200(SUCCESS)
	...
	function update_doctor_profile_put(){	
	...
	function update_doctor_mobile_put(){	
		...
		if($this->Doctor_model->uniqueId <> '' && $this->Doctor_model->doctorMobile <> '' &&  $this->Doctor_model->otp <> ''){
	...
	function verify_doctor_otp_put(){	
...
// Response		: if patient unique id is empty return status = 460 (INCOMPLETE INFORMATION), other wise update the patient basic details provided and  return status = 200(SUCCESS)
	...
	function update_patient_profile_put(){	
	...
	function update_patient_mobile_put(){	
		...
		if($this->Patient_model->uniqueId <> '' && $this->Patient_model->patientMobile <> '' &&  $this->Patient_model->otp <> ''){
	...
	function doctor_details_get(){	
		...
		if($this->Doctor_model->doctorId <> ''){
			...
			if(!$result){
				...
				$response = ['status' => $status,'result' => array(),'doctorId' => $result['doctorId'],'doctorUniqueId'  => $result['doctorUniqueId'],'doctorName'  => $result['doctorName'],'doctorEmail'  => $result['doctorEmail'],'doctorMobile'  => $result['doctorMobile'],'qualification'  => $result['qualification'],'doctorImageUrl'  => $result['doctorImageUrl'],'experience'  => $result['experience'],'knownLanguages'  => $result['knownLanguages'],'specialization'  => $result['specialization'],'youtubeLink'  => $result['youtubeLink'],'doctorAddress'  => $result['doctorAddress'],'communicationMode'  => $result['communicationMode'],'doctorFee'  => $result['doctorFee'],'doctorSessionDuration'  => $result['doctorSessionDuration'],'specialityName'  => $result['specialityName'],'num_of_rating'  => $result['num_of_rating'],'overall_rating'  => $result['overall_rating'],'isTrialCompleted'  => $result['isTrialCompleted'],'hasValidCredits'  => $result['hasValidCredits'],'patientCreditId'  => $result['patientCreditId'],'hasValidPackage'  => $result['hasValidPackage'],'subscriptionId'  => $result['subscriptionId'],'noOfSession'  => $result['noOfSession'],'sessionDuration'  => $result['sessionDuration'],'num_of_consultation'  => $result['num_of_consultation'],'isAvailable' => $isAvailable,'today_available_sessions' => $today_available_sessions,'available_sessions' => $available_sessions,'packages' => $packages,'rating' => $rating,'message' => HTTP_STATUS_CODES[200]];
...
// Response		: if member id or ratingTitle is empty return status = 460 (INCOMPLETE INFORMATION), and add the rating to system return status = 200(SUCCESS) 			
	...
	function update_appointment_status_put(){	
	...
	function complete_appointment_put(){	
	...
	function end_communication_log_put(){	
	...
	function update_payment_status_put(){	
