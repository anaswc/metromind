...
class  Subscription_model extends CI_Model {
	...
	public function getSubscription($limit = NULL, $start = NULL)
		{	
			...
			if($this->sortColumn == '')
				$this->sortColumn = "axsubscription.subscribedDate";
		...
		public function get_doctors_packages($doctorId)
		{	
			if($doctorId == '')
				return 0;
			
			$this->db->select('axpackages.*');
			...
			if($doctorId != "")
				$this->db->where('axpackages.doctorId', $doctorId);
	...
	public function getSubscription_id($subscriptionId = NULL)
		...
		public function get_count() {
		 ...
		 public function activate_subscription($subscriptionIds) { 
		
			$this->db->where_in("subscriptionId", $subscriptionIds); 
			$status = 1;
			$data = array(
				'status' => $status
			);
			$this->db->update("axsubscription", $data);  
		} 
		
		public function deactivate_subscription($subscriptionIds) { 
