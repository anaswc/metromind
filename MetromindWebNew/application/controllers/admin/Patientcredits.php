<?php
class Patientcredits extends CI_controller{
	 public function __construct(){
		...
		if ($this->input->post_get('returnUrl') <> ''){
			...
			$this->returnUrl= base_url('admin/patientcredits/');	
	 ...
	 public function index(){
			...
			$config["base_url"] = base_url() . "admin/patientcredits/";
			$total=$this->patients_model->get_patientCredits();
			...
			$data['patientcredit'] = $this->patients_model->get_patientCredits($limit, $limit*($start-1)); //echo "<pre>";print_r($data['adminuser']);echo "</pre>";exit;
			
		
			$this->load->view('admin/patientcredits/index', $data);
		}
