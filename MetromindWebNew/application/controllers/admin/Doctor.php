...
class Doctor extends CI_controller{
	 ...
	 public function index(){ 
		...
		$data['symptom'] = $this->symptom_model->getSymptom();
			...
			if ($this->input->post_get('doctorEmail') <> ''){
				$data['doctorEmail'] = $this->input->post_get('doctorEmail');
			}else{
				$data['doctorEmail'] = '';	
			}
...
//echo $availableDay ;exit;
			...
			$data['day'] = $availableDay;//print_r($data['day']);exit;
			$data['doctor_item'] = $this->doctor_model->getDoctor_id($doctorId);
			$data['day_item'] = $this->doctor_model->getDoctorDay_id($doctorId,$availableDay);
			//print_r($data['day_item']);exit;
			$data['available_item'] = $this->doctor_model->getAvailable_Id($doctorId);
			...
			$this->form_validation->set_rules('doctorId', 'doctorId', 'required');
			//$this->form_validation->set_rules('availableDay', 'availableDay', 'required');
				
			if ($this->form_validation->run() === FALSE)
			{	
			 $this->load->view('admin/doctor/availability', $data);
			}
	...
	public function checkEmail()
		{ 	
			$doctorEmail = $this->input->post_get('doctorEmail'); 
			$doctorId = $this->input->post_get('doctorId'); 
			 
			
			$data = $this->doctor_model->validateDoctorEmail($doctorEmail,$doctorId);
		...
		{ 	
			$doctorMobile = $this->input->post_get('doctorMobile'); 
			$doctorId = $this->input->post_get('doctorId'); 
			 
			
			$data = $this->doctor_model->validateDoctorMobile($doctorMobile,$doctorId);
			echo json_encode($data);
		}
