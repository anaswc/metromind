...
class Newrequest extends CI_controller{
	 public function __construct(){
		...
		if ($this->input->post_get('returnUrl') <> ''){
			...
			$this->returnUrl= base_url('admin/newrequest/');	
	 ...
	 public function index(){
			...
			$counts=$this->appointments_model->getnew_appointment_requests();
			...
			$data['appointments'] = $this->appointments_model->getnew_appointment_requests($limit, $limit*($start-1));
			
			//echo "<pre>";print_r($data['appointments']);exit;
			
		
			$this->load->view('admin/newrequest/index', $data);		}
	...
	public function create()
		...
		public function view($appointmentId = NULL)
		{
			...
			$this->load->view('admin/newrequest/view', $data);
		}
	...
	public function update($appointmentId = NULL)
		{ 
			...
			if ($this->form_validation->run() === FALSE)
			{	
			 $this->load->view('admin/newrequest/update', $data);
			}
			else
			{	
				...
				redirect(base_url('admin/newrequest'));
			 }
		   }
