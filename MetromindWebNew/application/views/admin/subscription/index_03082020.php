...
function addUser(){
  document.location.href= "<?php echo base_url('admin/subscription/create'); ?>?returnUrl=<?php echo urlencode(getCurrentPageURL())?>";
}
...
function submitSearch() 
  ...
  var checkFlag   = 0;
  var selectedValue = "";   
  if(frm.elements["subscriptionId[]"]!=null)
  {
    selectedValue   = getCheckedItem(frm,"subscriptionId[]");
    if (frm.elements["subscriptionId[]"].checked == true) 
    {   
      checkFlag = 1;
    }
    for(var i=0;i<frm.elements["subscriptionId[]"].length;i++) 
    {
      if (frm.elements["subscriptionId[]"][i].checked) 
      { 
        checkFlag++;    
      }   
    }   
  ...
  if (checkFlag == 0) 
  {
    ...
    {
      alert("Please select one subscription");
    }
  ...
  { 
    if (action == "DELETE") 
    {
      if(confirm("You are about to  "+action.toLowerCase()+"  the selected subscription(s). Do you wish to continue?")) 
      {
        frm.subscriptionIds.value  = selectedValue;
        frm.action.value  = action;
        frm.action      = "<?php echo base_url('admin/subscription/delete')?>?returnUrl=<?php echo urlencode(getCurrentPageURL())?>";
        frm.submit();     
      }
    }
    else if (action == "EDIT") 
    {
      ...
      {
        frm.subscriptionId.value = selectedValue;
        frm.action.value  = action;
        frm.action      = "<?php echo base_url()?>admin/subscription/update/?returnUrl=<?php echo urlencode(getCurrentPageURL())?>";
        frm.submit();     
      }
    }
...

    {
...

      {
        ...
        frm.action        = "<?php echo base_url('admin/subscription/activate')?>?returnUrl=<?php echo urlencode(getCurrentPageURL())?>";
...

    {
...

      {
        ...
        frm.action        = "<?php echo base_url('admin/subscription/deactivate')?>?returnUrl=<?php echo urlencode(getCurrentPageURL())?>";
