		...
		function checkAvailability(doctorId) {
			...
			$.ajax({
				...
				success: function (data) {
					...
					html +=
						' <h5 class="card-header-text"><i class="icofont icofont-ui-note"></i> Doctor Appoinments  (' +
						formatDate(appointmentDate) + ')</h5>'
					html += '</div>'
					if (data.length > 0) {
						...
						html += '<thead>'
						html += '<tr>'
						html += '<th>Sessions with patients</th>'
						html += '<th>Status</th>'
						// html += '<th>To</th>'
		...
		function checkPatient_availability() {
			...
			$.ajax({
				...
				success: function (data) {
					...
					html += '<div class="card-header">'
					html +=
						' <h5 class="card-header-text"><i class="icofont icofont-ui-note"></i> Patient Appoinments  (' +
						formatDate(appointmentDate) + ')</h5>'
					html += '</div>'
					if (data.length > 0) {
						html += '<div class="card-block" >'
									...
									<table class="table">
										...
										<tbody>
											<tr class="table-active">
												...
												<td><?php echo $appoinment_item['firstName']." ".$appoinment_item['lastName'] ?>
												</td>
												<!-- <td><?php echo changeDateFormat($appoinment_item['requestDate']) ?></td> -->
												<!-- <td><?php echo $appoinment_item['requestSession'] ?></td> -->
												<td><?php echo changeDateFormat($appoinment_item['appointmentDate']) ?>
												</td>
												<!-- <td><?php echo $appoinment_item['appointmentSession'] ?></td> -->
												<td><?php echo date('h:i:s a',strtotime($appoinment_item['appointmentStartTime'])) ?>
												</td>
												<td><?php echo date('h:i:s a',strtotime($appoinment_item['appointmentEndTime'])) ?>
												</td>
												<?php 	if($appoinment_item["status"]==1)
						{
							$appoinment_item["status"]="Approved";
						}
						else if($appoinment_item["status"]==0)
						{
							$appoinment_item["status"]="Requested";
						}
						else if($appoinment_item["status"]==2)
						{
							$appoinment_item["status"]="Rejected";
            }
            else if($appoinment_item["status"]==4)
						{
							$appoinment_item["status"]="Cancelled";
						}?>
												<td><?php echo $appoinment_item['status']; ?></td>
											</tr>
						...
						<div class="col-xl-4 col-lg-5 col-md-6 push-lg-7 push-xl-8 push-md-6 task-detail-right">
							...
							<div class="card" id="appoinmentToday">
								<div class="card-header">
									<h5 class="card-header-text"><i class="icofont icofont-ui-note"></i> Doctor
										Appointments
										(<?php echo changeDateFormat($appoinment_item['appointmentDate']) ?>)</h5>
								</div>
								...
								<div class="card-block">
									<table class="table table-borderless table-xs">
										...
										<tbody>
											...
											<tr>
												<td><?php echo date('h:i:s a',strtotime($row['appointmentStartTime'])).' - '.date('h:i:s a',strtotime($row['appointmentEndTime'])) ?></td>
                                                <td><?php echo $row['status'] ?></td>

												<!-- <td><?php echo date('h:i:s a',strtotime($row['appointmentStartTime'])).' - '.date('h:i:s a',strtotime($row['appointmentEndTime'])) ?> -->
												</td>
...
<!-- 
							...
							<div class="card" id="patientAvailability">
								<div class="card-header">
									<h5 class="card-header-text"><i class="icofont icofont-ui-note"></i> Patient
										Appoinments
										(<?php echo changeDateFormat($appoinment_item['appointmentDate']) ?>)</h5>
								</div>
									...
									<table class="table table-borderless table-xs">
										...
										<tbody>
											...
											<tr>
												...
												<td><?php echo date('h:i:s a',strtotime($row['appointmentStartTime'])) ?>
												</td>
												<td><?php echo date('h:i:s a',strtotime($row['appointmentEndTime'])) ?>
												</td>
											</tr>
											<?php } ?>
										</tbody>
						...
						<form name="frmAppoinment" id="frmAppoinment" method="post"
							action="<?php echo base_url('admin/appointment/update/'.$appoinment_item['appointmentId']) ?>"
							onSubmit="return validateAppointment(event)">
								...
								<div class="card">
									<div class="card-header">
										<h5 class="card-header-text"> <i
												class="icofont icofont-tasks-alt m-r-5"></i>Update appoinment</h5>
															...
															<tr>
																<td>
																	...
																	<input type="hidden" name="appointmentId"
																		id="appointmentId"
																		value="<?php echo $appoinment_item['appointmentId'] ?>">
																	<input type="hidden" name="patientId" id="patientId"
																		value="<?php echo $appoinment_item['patientId'] ?>">
									...
									foreach($doctorList as $doc){
									 ?>
																		<option value="<?php echo $doc['doctorId'] ?>"
																			<?php if($doc['doctorId']==$appoinment_item['doctorId']){  ?>
																			selected <?php } ?>>
																			<?php echo $doc['doctorName'] ?></option>
																		<?php }
								}?>
																	</select>
																</td>
																<td><input type="date" name="appointmentDate"
																		...
																		value="<?php echo $appoinment_item['appointmentDate']  ?>"
																		required onChange="checkAvailability();"></td>
																<!-- <td>
 ...
 <select name="appointmentSession" id="appointmentSession"  class="form-control" required>
							...
							//	}?>
																...
																<td> <input type="time" name="appointmentStartTime"
																		id="appointmentStartTime" min="08:59:00"
																		max="20:59:00" class="form-control"
																		value="<?php echo $appoinment_item['appointmentStartTime']  ?>"
																		required></td>
                                                                ...
                                                                <select name="status"
                                                                        ...
                                                                        <option value="0"  <?php  if($appoinment_item['status']=="Requested"){ echo "selected"; }?>>Requested</option>
                                                                        <option value="1" <?php  if($appoinment_item['status']=="Approved"){ echo "selected"; }?>>Approved</option>
                                                                        <option value="2"<?php  if($appoinment_item['status']=="Rejected"){ echo "selected"; }?>>Rejected</option>
                                                                        <option value="4" <?php  if($appoinment_item['status']=="Cancelled"){ echo "selected"; }?>>Cancelled</option>
											...
											<div class="col-md-12 col-xs-12">
												<div class="form-group row">
													<div align="center">
														<button type="submit"
															class="btn btn-mini btn-primary waves-effect waves-light m-r-30">Submit</button>
														<button type="button"
															class="btn btn-mini btn-primary waves-effect waves-light m-r-30"
															onclick="javascript:document.location.href='<?php echo base_url('admin/appointment'); ?>';">Cancel</button>
													</div>
												</div>
											</div>
