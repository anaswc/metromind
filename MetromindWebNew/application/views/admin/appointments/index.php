	...
	<script language="javascript">
		function addAppointment() {
			document.location.href =
				"<?php echo base_url('admin/appointment/create'); ?>?returnUrl=<?php echo urlencode(getCurrentPageURL())?>";
		}
		...
		function submitPage() {
			...
			frm.action = "<?php echo base_url()?>admin/appointment" +
				...
				+
				"&sortDirection=<?php echo $this->appointments_model->sortDirection?>&sortColumn=<?php echo $this->appointments_model->sortColumn?>&pageSize=" +
				pageSize;
				...
				<div class="col-sm-12 p-0">
					<div class="main-header">
						...
						<ol class="breadcrumb breadcrumb-title breadcrumb-arrow">
							<li class="breadcrumb-item"><a href="<?php echo base_url('admin/dashboard'); ?>"><i
				...
				<div class="col-sm-12">
					<div class="card">
						<div class="card-block">
							<form name="frmSymptomSearch" method="post" action="" class="form-inline"
								onsubmit="return submitSearch();">
								<input type="hidden" name="clsaxSpeciality_submitted" value="">
								<div class="form-group m-r-15">
									<input type="text" class="form-control input-rounded" name="doctorName" placeholder="Doctor Name"
										value="<?php echo $this->appointments_model->doctorName?>"></div>
								<div class="form-group m-r-15">
									<input type="text" class="form-control input-rounded" name="firstName" placeholder="Patient Name"
										value="<?php echo $this->appointments_model->firstName?>"></div>

								<div class="form-group m-r-15">
									<input type="date" class="form-control input-rounded" name="appointmentDate"
										placeholder="Appointment Date" value="<?php echo $this->appointments_model->appointmentDate?>">
								</div>
								<div class="form-group m-r-15">
								...
								<div class="form-group m-r-15">
									<select name="status" class="form-control input-rounded">
										<option value="" <?php  if($this->appointments_model->status==""){ echo "selected"; }?>>--Select--
										</option>
										<option value="0" <?php  if($this->appointments_model->status=="0"){ echo "selected"; }?>>Requested
										</option>
										<option value="1" <?php  if($this->appointments_model->status=="1"){ echo "selected"; }?>>Approved
										</option>
										<option value="2" <?php  if($this->appointments_model->status=="2"){ echo "selected"; }?>>Rejected
										</option>
										<option value="4" <?php  if($this->appointments_model->status=="4"){ echo "selected"; }?>>Cancelled
										</option>
								...
								<button type="button" class="btn btn-mini btn-primary waves-effect waves-light m-r-30"
						...
						<div class="card-block">
							...
							<button type="button" class="btn btn-mini btn-primary waves-effect waves-light m-r-30"
								onclick="addAppointment();">Add New</button>
										...
										<thead>
											<tr>
												...
												<th width="2%" height="24" class="nosort">
													<input type="checkbox" name="chkAll" value="checkbox" class="checkall"
														onClick="selectAll(appointmentId, this);">
												</th>
												<th>No</th>
                    ...
                    ?>
											<tr>
												<td valign="top" class="rowcontent">
													<input type="checkbox" name="appointmentId[]" value="<?php echo $row["appointmentId"]?>"
														id="appointmentId"
														onCLick='resetChkBox(appointmentId, chkAll);changeCheckedColor(frmAppointmentList.appointmentId);' />
												</td>

												<td><?php echo $cnt?></td>
												<td> <a href="<?php echo base_url('admin/appointment/view/'.$row["appointmentId"])   ?>"
														data-original-title="View"><i class="ion-eye"></i> </a>&nbsp;
													<?php if($completed<>1){?>
													<a href="<?php echo  base_url('admin/appointment/update/'.$row["appointmentId"]).'/'  ?>?returnUrl=<?php echo urlencode(getCurrentPageURL())?>"
														data-original-title="Edit"><i class="icofont icofont-edit"></i> </a>
													<?php }
                    ...
                    ?>
									...
									<div class="card-block">
										...
										<button class="btn btn-mini btn-primary waves-effect waves-light m-r-30"
											onClick="submitPage();"><span>Go</span></button>
									</div>
									<div class="card-block">
